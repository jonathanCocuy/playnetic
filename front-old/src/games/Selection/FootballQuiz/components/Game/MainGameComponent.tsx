...
interface FootballQuizComponentProps {
    levelCount: number;
    setLevelCount: (level: number) => void;
}

const FootballQuizComponent = ({ levelCount, setLevelCount }: FootballQuizComponentProps) => {
    ...
    const currentTitle = () => {
        if (isFirstLevel) {
            ...
            return dataLevel1[exerciseIndex]?.title || "Lo básico";
        } else if (isSecondLevel) {
            const exerciseIndex = levelCount - 6; // Convertir levelCount a índice (6->0, 7->1, etc.)
            return dataLevel2[exerciseIndex]?.title || "Conocimientos intermedios";
        } else if (isThirdLevel) {
            const exerciseIndex = levelCount - 11; // Convertir levelCount a índice (11->0, 12->1, etc.)
            return dataLevel3[exerciseIndex]?.title || "Curiosidades y retos";
        }
        return "¡Felicidades!";
    };

    return (
        <div className="football-quiz">
            <button 
...
export default FootballQuizComponent;
